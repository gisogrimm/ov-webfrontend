...
function update_devicestatus( user, device, owned_devices ) {
  ...
  if ( device.showexpertsettings ) {
    ...
    request.onload = function() {
      ...
      if ( ( log.length > 2 ) && ( device.logage < device.age ) ) {
        logwin.replaceChildren( document.createTextNode( log ) );
        logwin.style = 'display: block;';
        logwin.focus();
        logwin.setSelectionRange( logwin.value.length, logwin.value.length );
      } else {
...
function update_gainhints( gainhint, levelstats, device ) {
  ...
  if ( show ) {
    ...
    gainhint.appendChild( document.createTextNode( msg ) );
  } else {
...
function everytenseconds() {
  ...
  var gainhint = document.getElementById( 'gainhint' );
...
function starttimer() {
  ...
  everytenseconds();
  timer10 = setInterval( everytenseconds, 6000 );
}
