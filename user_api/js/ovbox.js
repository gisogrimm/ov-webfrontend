...
function rest_set_devprop(name, value) {
  ...
  if (typeof value === "boolean") request.send('setdevpropbool=' + name + '&' +
    name + '=' + value);
  else if (typeof value === "number") request.send('setdevpropfloat=' + name +
    '&' + name + '=' + value);
...
function rest_set_userprop(name, value, reload = false) {
  ...
  if (typeof value === "boolean") request.send('setuserpropbool=' + name + '&' +
    name + '=' + value);
  else if (typeof value === "number") request.send('setuserpropfloat=' + name +
    '&' + name + '=' + value);
...
function update_room(user, device, room, droom) {
  ...
  for (const chair in room.roomdev) {
    ...
    if (device.chair != chair) {
      if (device.peer2peer && dev.peer2peer) {
        ...
        if (room.lat.hasOwnProperty(latkey1) && room.lat.hasOwnProperty(
          ...
          // only display values from last 15 minutes:
          if ((room.now - room.lat[latkey1].access < 900) && (room.now - room
              .lat[latkey2].access < 900)) {
            const lat = 0.5 * (Number(room.lat[latkey1].lat) + Number(room.lat[
              latkey2].lat)) + Number(dev.jittersend) + Number(device
              .jitterreceive) + 10;
            latdisp = ' ' + lat.toFixed(1) + 'ms ';
          }
        }
    ...
    if (mtype == 'a') memlink.setAttribute('href', '?swapdev=' + encodeURI(dev
      .id));
    var lab = dev.label;
...
function update_devicestatus(user, device, owned_devices) {
  ...
  if (device.id.length == 0) {
    ...
    var lastseen = '';
    var otherdev = '';
    if (device.firmwareupdate) lastseen = ' Firmware update pending. ';
    if (device.isupdating) {
      if (device.age < 2000) lastseen =
    ...
    if (device.age < 20) {
      if (device.bandwidth && ((device.bandwidth.tx > 0) || (device.bandwidth
          .rx > 0))) {
        var txstr;
        if (device.bandwidth.tx >= 100000) txstr = (0.000001 * device.bandwidth
          .tx).toFixed(2) + ' MBps';
        else txstr = (0.001 * device.bandwidth.tx).toFixed(2) + ' kBps';
        var rxstr;
        if (device.bandwidth.rx >= 100000) rxstr = (0.000001 * device.bandwidth
          .rx).toFixed(2) + ' MBps';
      ...
      if (device.backendperiodsize && (device.backendperiodsize > 0) && device
        .backendsrate && (device.backendsrate > 0)) {
  ...
  if (devsel) {
    ...
    for (const od in owned_devices) {
      var act = '';
      if ((owned_devices[od].age < 20) && (od != device.id)) act = ' *active*';
...
function update_unclaimed(user, unclaimed_devices) {
  ...
  if (unclaimed_devices.length == 0) p.setAttribute('style', 'display:none;');
  else {
    ...
    for (var k = 0; k < unclaimed_devices.length; k++) {
      ...
      form.appendChild(document.createElement('button')).appendChild(document
        .createTextNode(unclaimed_devices[k]));
    }
  }
}
var tstart;
...
function everysecond() {
  ...
  for (var k = 0, len = el.length | 0; k < len; k = k + 1 | 0) {
    ...
    el[k].appendChild(document.createTextNode(Math.floor(0.001 * (Date.now() -
      tstart))));
  }
}
...
function create_row_stat(tab, chair, data) {
  ...
  else td.appendChild(document.createTextNode(chair + ' ' + data.names[dev]));
  td.setAttribute('class', 'statcellrec');
  return tr;
}
  ...
  for (const chair in data.chairs) {
    ...
    for (var k = 0; k < data.n; k++) {
      if (matelem(mat_jitter, chidx[chair], k, data.n) > 0) {
        var sq = Math.sqrt(sqr(matelem(mat_jitter, chidx[chair], k, data.n)) -
          sqr(vjitrec[chairidx[k]]));
        jitsend = Math.max(jitsend, sq);
      }
    }
    vjitsend[chair] = Math.ceil(Math.sqrt(sqr(jitsend) + sqr(data.fragsize[data
      .chairs[chair]])));
  }
  ...
  request.onload = function() {
    ...
    if (data && data.stats && (data.room.length > 0)) {
      ...
      for (const mode in modes) {
          ...
          if (dev && data.stats[dev]) {
            for (const chx in data.chairs) {
              if (data.stats[dev][chx] && data.stats[dev][chx][modes[mode]])
                if (data.stats[dev][chx][modes[mode]].received > 0) show =
                  true;
            }
          }
        }
        if (show) {
          {
            var tab = create_tab_stat(div, 'Suggestions ' + modes[mode], data,
              false);
          ...
          for (const chair in data.chairs) {
            ...
            if (dev && data.stats[dev]) {
              ...
              for (const chx in data.chairs) {
                var pt = -1;
                if (data.stats[dev][chx]) pt = data.stats[dev][chx][modes[
                  mode]].median;
                var td = tr.appendChild(document.createElement('td'));
                td.setAttribute('class', 'statcell');
                if (pt > 0) {
              ...
              for (const chx in data.chairs) {
                var pt = -1;
                if (data.stats[dev][chx]) pt = data.stats[dev][chx][modes[
                  mode]].p99 - data.stats[dev][chx][modes[mode]].min;
                var td = tr.appendChild(document.createElement('td'));
                td.setAttribute('class', 'statcell');
                if (pt > 0) {
                  td.appendChild(document.createTextNode(pt.toFixed(1) +
...
function update_jack_rate(rate) {
  ...
  document.getElementById('jackperiod').value = 16 * Math.floor(0.002 * rate /
    16);
}
...
function apply_jack_settings() {
  ...
  request.open('POST', '/rest.php', true);
  request.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
  var data = 'jackaudio=&jackplugdev=' + document.getElementById('jackplugdev')
...
function vid_toggle_max(id) {
  ...
  if (el && elthis && elfr) {
    if (el.className == 'sessionvid') {
      ...
      var x = document.getElementsByClassName('vidcont');
      for (var k = 0; k < x.length; k++) x[k].style.display = "inline-block";
 ...
 * Local Variables:
 * c-basic-offset: 2
 * compile-command: "js-beautify -d -s 2 -w 80 -r ovbox.js"
 * End:
 */
